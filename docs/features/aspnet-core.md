...
- **ProvaServiceCollection**: Handles the lifecycle of `WebApplicationFactory`. It automatically calls `Dispose()` on the factory when the test run completes, ensuring resources are freed.
