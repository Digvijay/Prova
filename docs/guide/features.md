...
Native LCOV support without external tools. See [Code Coverage](/guide/coverage) for details.
...
## 🔁 Flow Control
Fine-tune execution with Retry, Repeat, and per-test Culture settings. See [Flow Control](/guide/flow-control) for details.
...
## ⏱️ Timeout
Enforce maximum execution time for tests. See [Timeouts](/guide/timeouts) for details.
...
Fine-grained parallelism control. See [Resource Constraints](/guide/resource-constraints) for details.
...
## 🛠️ Data Generators
Custom, reusable data sources. See [Data Generators](/guide/data-generators) for details.
...
## 💉 DI Data Sources
Source test data from services registered in the DI container. See [DI Data Sources](/features/di-data-sources) for details.
...
```
## 🔄 Lifecycle Hooks
Granular control with Test, Class, and Assembly-level setup/teardown. See [Lifecycle Hooks](/guide/lifecycle-hooks) for more info.
...
## 🏭 Class Factories
Control test class instantiation for custom DI or complex logic. See [Class Factories](/guide/class-factories) for more info.
...
Seamless integration with `WebApplicationFactory` for AOT-compatible end-to-end tests. See [ASP.NET Core](/features/aspnet-core) for details.
